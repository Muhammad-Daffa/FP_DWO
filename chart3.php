...
<html lang="en">
    <head>
        ...
        <title>Pendapatan Tiap Kategori Per Tahun</title>
    ...
    <body class="sb-nav-fixed">
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Chart Pendapatan Tiap Kategori Per Tahun</h1>
                        <div class="card mb-4">
                        </div>
                        <div class="card mb-4">
                            <div class="card-header">
                                <i class="fas fa-chart-pie me-1"></i>
                                Chart Jumlah Pendapatan Tiap Tahun Berdasarkan Kategori
                            </div>
                            <?php
                                        $host       = "localhost";
                                        $user       = "root";
                                        ...
                                        $sql = "SELECT CONCAT('name:',t.year) as year, CONCAT('y:', SUM(fs.line_total)*100/" . $tot_amount[0] .") as y, CONCAT('drilldown:', t.year) as drilldown 
                                        FROM timedimens t
                                        ...
                                        $sql = "SELECT t.year year, p.category category, SUM(fs.line_total) as total
                                        FROM timedimens t
                                        JOIN fact_sales fs ON (t.time_id = fs.time_id)
                                        JOIN product p ON (p.product_id = fs.product_id)
                                        GROUP BY year, category";
                                        $det_month = mysqli_query($mysqli,$sql);

                                        while ($row = mysqli_fetch_all($det_month)) {
                                            $data_det[] = $row;
                                        }
        ...
        <script type="text/javascript">
            ...
            title: {
                text: 'Persentase Pendapatan Dari Kategori Barang Terjual Berdasarkan Tahun'},
                subtitle: {
                    text:'Klik untuk melihat detail jumlah dari kategori barang pada setiap tahunnya'
                },
                accessibility: {
                    announceNewData: {
                        enabled: true
