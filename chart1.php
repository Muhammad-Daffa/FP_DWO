...
<html lang="en">
    <head>
        ...
        <title>Kuantitas Per Kategori</title>
    ...
    <body class="sb-nav-fixed">
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Chart Kuantitas Per Kategori</h1>
                        <div class="card mb-4">
                        </div>
                        <div class="card mb-4">
                            <div class="card-header">
                                <i class="fas fa-chart-pie me-1"></i>
                                Chart Jumlah Barang Terjual Berdasarkan Kategori
                            </div>
                            <?php
                                        $host       = "localhost";
                                        $user       = "root";
                                        ...
                                        $sql = "SELECT SUM(order_quantity) AS tot_qty FROM fact_sales";
                                        $tot = mysqli_query($mysqli, $sql);
                                        $tot_amount = mysqli_fetch_row($tot);

                                        $sql = "SELECT CONCAT('name:',p.category) as category, CONCAT('y:', SUM(fs.order_quantity)*100/" . $tot_amount[0] .") as y, CONCAT('drilldown:', p.category) as drilldown 
                                        FROM product p
                                        ...
                                        $sql = "SELECT p.category category, p.subcategory subcategory, SUM(fs.order_quantity) as qty
                                        FROM product p
                                        JOIN fact_sales fs ON (p.product_id = fs.product_id)
        ...
        <script type="text/javascript">
            ...
            title: {
                text: 'Persentase Jumlah Barang Terjual Berdasarkan Kategori'},
                subtitle: {
                    text:'Klik untuk melihat detail jumlah dari setiap subkategori'
                },
                accessibility: {
                    announceNewData: {
                        enabled: true
