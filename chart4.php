...
<html lang="en">
    <head>
        ...
        <title>Transaksi Per Wilayah Bagian</title>
    ...
    <body class="sb-nav-fixed">
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Chart Transaksi Per Wilayah Bagian</h1>
                        <div class="card mb-4">
                        </div>
                        <div class="card mb-4">
                            <div class="card-header">
                                <i class="fas fa-chart-pie me-1"></i>
                                Chart Jumlah Pendapatan Berdasarkan Bagian/Provinsi Dan Kota
                            </div>
                            <?php
                                        $host       = "localhost";
                                        $user       = "root";
                                        ...
                                        $sql = "SELECT SUM(total_due) AS total FROM fact_shipment";
                                        $tot = mysqli_query($mysqli, $sql);
                                        $tot_amount = mysqli_fetch_row($tot);

                                        $sql = "SELECT CONCAT('name:',a.state_province) as state_province, CONCAT('y:', SUM(fs.total_due)*100/" . $tot_amount[0] .") as y, CONCAT('drilldown:', a.state_province) as drilldown 
                                        FROM address a
                                        JOIN fact_shipment fs ON (a.address_id = fs.address_id) 
                                        GROUP BY state_province";
                                        $all_state = mysqli_query($mysqli,$sql);
                                        while($row = mysqli_fetch_all($all_state)) {
                                            $data[] = $row;
                                        }
                                        $json_all_state = json_encode($data);

                                        // Chart Kedua

                                        $sql = "SELECT a.state_province state_province, a.city city, SUM(fs.total_due) as total
                                        FROM address a
                                        JOIN fact_shipment fs ON (a.address_id = fs.address_id)
                                        GROUP BY state_province, city";
                                        $det_city = mysqli_query($mysqli,$sql);

                                        while ($row = mysqli_fetch_all($det_city)) {
                                            $data_det[] = $row;
                                        }
        ...
        <script type="text/javascript">
            Highcharts.chart('chart1', {
                ...
                title: {
                    text: 'Persentase Transaksi Dari Pengiriman Berdasarkan Wilayah Bagian/Provinsi dan Kota'},
                    subtitle: {
                        text:'Klik untuk melihat detail jumlah dari setiap kota'
                    },
                    accessibility: {
                        announceNewData: {
                            enabled: true
